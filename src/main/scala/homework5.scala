...
object homework5 extends App {
  ...
  val veggie: Map[Char, Array[String]] = Veggies.items.groupBy(_.charAt(0))
  val sortedVeggie = veggie.toSeq.sortBy(_._1)
  for ((key, value) <- veggie) {
    println(s"Letter $key -> ${value.toList.sorted}")
  }

  println(veggie.view.mapValues(x=>x.size).mkString(", ")) //prints how many veggies start with each letter

  for((key, value) <- sortedVeggie) {
    if (value.toList.size > 1) {
      println(s"There are ${value.toList.size} vegetables that start with $key:\n${value.mkString(", ")}\n")
    } else {
      println(s"There is ${value.toList.size} vegetables that start with $key:\n${value.mkString(", ")}\n")
    }
  }

  val groupedVeggies = Veggies.items.groupBy(_.head)
  val sortedVeggies = groupedVeggies.to(collection.immutable.SortedMap)
  for ((key, value) <- sortedVeggies) {
    println(s"Letter $key -> ${value.length} veggie(s) ${value.toList.sorted}")
  }


  val storedFruits = Array("apple", "apple", "pear", "orange")
  val exactGrouping = storedFruits.groupBy(_.toString)
  for ((key, value) <- exactGrouping) println(s"Key - $key -> ${value.toList}")
  // this is a faster way of obtaining counts for each unit
  val fruitCounts = exactGrouping.map(keyvalue => (keyvalue._1, keyvalue._2.length))
  println(fruitCounts)
}
